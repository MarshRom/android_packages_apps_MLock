...
public class KeyguardViewDragHelper {
    ...
    private static final String TAG = "KeyguardViewDragHelper";
    ...
    private final Runnable mSetIdleRunnable = new Runnable() {
        public void run() {
            KeyguardViewDragHelper.this.setDragState(KeyguardViewDragHelper.STATE_IDLE);
        }
    };
    ...
    public static abstract class Callback {
        ...
        public int getViewHorizontalDragRange(View child) {
            return KeyguardViewDragHelper.STATE_IDLE;
        }
    ...
    public static KeyguardViewDragHelper create(ViewGroup forParent, Callback cb) {
        return new KeyguardViewDragHelper(forParent.getContext(), forParent, cb);
    }

    public static KeyguardViewDragHelper create(ViewGroup forParent, float sensitivity, Callback cb) {
        KeyguardViewDragHelper helper = create(forParent, cb);
        helper.mTouchSlop = (int) (((float) helper.mTouchSlop) * (1.0f / sensitivity));
        return helper;
    }

    private KeyguardViewDragHelper(Context context, ViewGroup forParent, Callback cb) {
        if (forParent == null) {
    ...
    public void captureChildView(View childView, int activePointerId) {
        if (childView.getParent() != this.mParentView) {
            throw new IllegalArgumentException("captureChildView: parameter must be a descendant of the KeyguardViewDragHelper's tracked parent view (" + this.mParentView + ")");
        }
    ...
    public boolean smoothSlideViewTo(View child, int finalLeft, int finalTop) {
        ...
        this.mActivePointerId = INVALID_POINTER;
        return forceSettleCapturedViewAt(finalLeft, finalTop, STATE_IDLE, STATE_IDLE, INVALID_POINTER);
    }

    public boolean smoothSlideViewTo(View child, int finalLeft, int finalTop, int duration) {
        this.mCapturedView = child;
        this.mActivePointerId = INVALID_POINTER;
        return forceSettleCapturedViewAt(finalLeft, finalTop, STATE_IDLE, STATE_IDLE, duration);
    }

    public boolean settleCapturedViewAt(int finalLeft, int finalTop) {
        if (this.mReleaseInProgress) {
            return forceSettleCapturedViewAt(finalLeft, finalTop, (int) VelocityTrackerCompat.getXVelocity(this.mVelocityTracker, this.mActivePointerId), (int) VelocityTrackerCompat.getYVelocity(this.mVelocityTracker, this.mActivePointerId), INVALID_POINTER);
        }
    ...
    private boolean forceSettleCapturedViewAt(int finalLeft, int finalTop, int xvel, int yvel, int duration) {
        ...
        if (duration < 0) {
            duration = computeSettleDuration(this.mCapturedView, dx, dy, xvel, yvel);
        }
    ...
    L_0x02b8:
        ...
        goto L_0x002f;
        */
        throw new UnsupportedOperationException("Method not decompiled: com.cyngn.keyguard.KeyguardViewDragHelper.processTouchEvent(android.view.MotionEvent):void");
    }
    ...
    public View findTopChildUnder(int x, int y) {
        View child = this.mCallback.findTopChildUnder(x, y);
        if (child != null) {
            return child;
        }
        for (int i = this.mParentView.getChildCount() + INVALID_POINTER; i >= 0; i += INVALID_POINTER) {
            child = this.mParentView.getChildAt(this.mCallback.getOrderedChildIndex(i));
