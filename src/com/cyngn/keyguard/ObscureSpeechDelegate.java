...
class ObscureSpeechDelegate extends AccessibilityDelegate {
    ...
    public void sendAccessibilityEvent(View host, int eventType) {
        ...
        if (eventType == 32768 && !sAnnouncedHeadset && shouldObscureSpeech()) {
            ...
            host.announceForAccessibility(host.getContext().getString(17040906));
        }
    }
    ...
    public void onPopulateAccessibilityEvent(View host, AccessibilityEvent event) {
        ...
        if (event.getEventType() != 16384 && shouldObscureSpeech()) {
            event.getText().clear();
            event.setContentDescription(host.getContext().getString(17040907));
        }
    }
    ...
    public void onInitializeAccessibilityNodeInfo(View host, AccessibilityNodeInfo info) {
        ...
        if (shouldObscureSpeech()) {
            ...
            info.setText(null);
            info.setContentDescription(ctx.getString(17040907));
        }
    }

    private boolean shouldObscureSpeech() {
        if (Secure.getInt(this.mContentResolver, "speak_password", 0) != 0 || this.mAudioManager.isWiredHeadsetOn() || this.mAudioManager.isBluetoothA2dpOn()) {
            return false;
        }
        return true;
    }
